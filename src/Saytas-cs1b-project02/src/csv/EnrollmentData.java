...
public class EnrollmentData
{
    ...
    public EnrollmentData(int numRows, int numColumns, int startingYear)
    {
        ...
        for(int i = 0; i < numColumns; i++)
        {
            ...
            //System.out.println("YEAR LABELS: " + yearLabels[i]);
        }
    }
     ...
     * @param partialPrimaryEnrollment  One dimensional array of double for the
     *                                  enrollment data of a specific country
     */
    public void addCountry(String countryName, double[] partialPrimaryEnrollment)
    {
        try
        {
     ...
     */
    public double[] getEnrollmentInCountryForPeriod(String countryName, int requestedStartYear, int requestedEndYear) throws IllegalArgumentException
    {
        double[] output;
        int countryIndex = -1;
        ...
        output = new double[endingPos - startingPos + 1];

        for(int i = startingPos, j = 0; i <= endingPos; i++, j++)
        {
            output[j] = table[countryIndex][i];
        }
        return output;
    }

    /**
     * String representation of the enrollment data stored in table[][]
     ...
     * @return  result which prints out a data table with country names
    ...
    public String toString()
    {
        ...
        for(int i = 0; i < countryNames.length; i++)
        {
            ...
            for(int j = 0; j < table[i].length; j++)
            {
                ...
                {
                    result += String.format("%10s ", String.format("%.2f", table[i][j]));
                }
            }
            result += "\n";
        }
