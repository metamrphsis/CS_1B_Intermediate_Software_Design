...
public class MagazineSubscriptions
{
	...
	public MagazineSubscriptions(MagazineSubscriptions original)
	{
		...
		//int size = original.getNumberOfSubscribers();
		this.customers = new Customer[MAX_SIZE];
		for(int i = 0; i < customers.length; i++)
		{
			try
			{
				//customers[i] = new Customer(customers[i]); // Not sure
				customers[i] = new Customer(original.customers[i]);
			}
...
//		for(int i = 0; i < customers.length; i++)
//		{
//			this.addCustomer(new Customer(original.customers[i]));
//		}
	}
